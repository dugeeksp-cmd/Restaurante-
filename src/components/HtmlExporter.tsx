...
<body class="bg-slate-900 min-h-screen pb-12 text-slate-100">
  ...
  return (
    <div id="html-exporter-root" class="bg-white rounded-3xl p-6 border-2 border-slate-200 shadow-xl max-w-4xl mx-auto">
      <div class="flex items-center gap-3 mb-4 border-b border-slate-100 pb-4">
        ...
        <div>
          <h2 class="text-xl font-black text-slate-800">Exportar Códigos Standalone</h2>
          <p class="text-xs text-slate-500 font-medium">Baixe ou copie o código completo das páginas HTML integradas com Firebase.</p>
        </div>
      </div>

      <p class="text-sm text-slate-600 mb-6 leading-relaxed">
        Ambos os arquivos já estão salvos e prontos para uso no seu projeto como <code class="bg-slate-100 text-indigo-600 px-1.5 py-0.5 rounded font-mono text-xs font-bold">/garcom.html</code> e <code class="bg-slate-100 text-indigo-600 px-1.5 py-0.5 rounded font-mono text-xs font-bold">/cozinha.html</code>. 
      ...
      {/* Tabs */}
      <div class="flex items-center justify-between gap-2 mb-4 bg-slate-100 p-1.5 rounded-2xl">
        <div class="flex gap-1 flex-1">
          <button
            onClick={() => setActiveTab('garcom')}
            class={`flex-1 sm:flex-initial text-center px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'garcom'
        ...
        <div class="flex gap-2">
          <button
            ...
            class="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3.5 py-2 rounded-xl shadow-sm transition-all active:scale-95 cursor-pointer"
          >
          ...
          <button
            ...
            class="hidden sm:flex items-center gap-1.5 bg-slate-800 hover:bg-slate-900 text-white text-xs font-bold px-3.5 py-2 rounded-xl shadow-sm transition-all active:scale-95 cursor-pointer"
          >
            <Download size={14} />
            Baixar Arquivo
          </button>
      ...
      <div class="bg-slate-950 rounded-2xl p-4 overflow-x-auto border border-slate-800 max-h-80 font-mono text-xs text-slate-300">
        <div class="flex justify-between items-center mb-2 border-b border-slate-900 pb-2">
          <span class="text-slate-500 text-[10px] uppercase font-bold tracking-wider">{activeTab}.html</span>
          <span class="text-emerald-500 text-[10px] font-bold">100% Completo & Standalone</span>
        </div>
...
...
  <script type="module">
    ...
    // Sons de frigideira sizzling e novos pedidos embutidos!
      ...
      <div class="mt-4 p-4 bg-amber-50 border border-amber-200/50 rounded-2xl flex items-start gap-3">
        <span class="text-xl">💡</span>
        <div class="text-xs text-amber-800 leading-relaxed font-medium">
          <p class="font-bold mb-1">Como testar as páginas locais no seu celular:</p>
          <ol class="list-decimal list-inside space-y-1">
            <li>Abra os arquivos <code class="font-bold">/garcom.html</code> e <code class="font-bold">/cozinha.html</code>.</li>
            <li>Substitua o objeto <code class="font-bold">firebaseConfig</code> com as chaves do seu projeto Firebase (basta criar um aplicativo da web no painel do Firebase).</li>
